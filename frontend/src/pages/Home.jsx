    ...
    const fetchBooks = async () => {
      ...
      try {
        const [featuredRes, bestsellersRes, newArrivalsRes] = await Promise.all(
          [
            axios.get(
              "http://localhost:5000/api/products?sort_by=publication_year&order=desc&limit=5"
            ),
            axios.get(
              "http://localhost:5000/api/products?sort_by=unit_price&order=desc&limit=5"
            ),
            axios.get(
              "http://localhost:5000/api/products?sort_by=created_at&order=desc&limit=5"
            ),
          ]
        );
